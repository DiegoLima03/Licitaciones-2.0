...
function sanitize_internal_path(?string $path, string $fallback = 'dashboard.php'): string
{
 ...
 *
 * Acepta datos tanto de la tabla `usuarios` (login legacy) como de
 * `profiles` (gestion de usuarios). Normaliza las claves para que
 * el resto de la app siempre encuentre: id, email, full_name, role.
 *
...
function login_user(array $user): void
{
  ...
  $username = (string)($user['username'] ?? $user['nombre_usuario'] ?? '');
  $email    = (string)($user['email'] ?? $username);
  $fullName = (string)($user['full_name'] ?? $user['nombre_completo'] ?? $username);
  $isAdmin  = ((int)($user['is_admin'] ?? $user['es_admin'] ?? 0) === 1);

  // Rol: si viene explicito lo usamos; si no, derivamos de is_admin
  $role = (string)($user['role'] ?? '');
  if ($role === '') {
    $role = $isAdmin ? 'admin' : 'member_licitaciones';
  }
...
function require_login(string $defaultTarget = 'dashboard.php'): void
{
  if (AUTH_BYPASS) {
    return;
  }
