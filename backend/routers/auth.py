"""
Autenticación: Supabase Auth + fallback tbl_usuarios (legacy).
...
LEGACY_JWT_AUDIENCE = "veraleza-legacy"
...
def _get_or_create_profile(user_id: str, email: str, default_org_id: str) -> dict[str, Any]:
    ...
    # Perfil inexistente: crear con org por defecto (usuarios legacy)
    ...
    return {"organization_id": default_org_id, "role": "member", "full_name": None}


def _login_legacy_tbl_usuarios(email: str, password: str) -> dict | None:
    """Fallback: autenticación contra tbl_usuarios (antes de migración SaaS)."""
    try:
        ...
        # Obtener org por defecto (si existe organizations)
        ...
        # Generar JWT legacy para que get_current_user lo acepte
        access_token = None
        if SUPABASE_JWT_SECRET:
            payload = {
                ...
                "org_id": org_id,
                "exp": int(time.time()) + 86400 * 7,  # 7 días
            }
        ...
        return {
            ...
            "role": usuario.get("rol") or "member",
            "rol": usuario.get("rol") or "member",
    ...
    if auth_resp and auth_resp.session and auth_resp.user:
        ...
        if default_org_id:
            ...
            return {
                ...
                "rol": profile.get("role") or "member",
                "access_token": session.access_token,
            }

    # 2. Fallback: tbl_usuarios (antes de migración)
    legacy = _login_legacy_tbl_usuarios(payload.email, payload.password)
    if legacy:
        return legacy
...
class UserCreatePayload(UserLogin):
    """Payload para crear usuario (email, password + opcional full_name)."""
    full_name: str | None = None


class UserRoleUpdate(BaseModel):
    """Payload para actualizar el rol de un usuario."""
    role: str = Field(..., description="Nuevo rol: admin o member.")


ROLES_VALIDOS = {"admin", "member"}
...
def create_user(
    ...
    POST /auth/users
    Body: { "email": "...", "password": "...", "full_name": "..." (opcional) }
    ...
    # Asignar el nuevo usuario a la misma organización que el admin
    try:
        ...
        pass  # El trigger ya creó el perfil; si falla el update, el usuario tiene org por defecto
...
def update_user_role(
    ...
    if payload.role not in ROLES_VALIDOS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Rol inválido. Valores permitidos: {sorted(ROLES_VALIDOS)}",
        )
